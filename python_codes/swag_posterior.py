# This code is obtained from https://github.com/wjmaddox/swa_gaussian. This GitHub repository is based on the paper: 'A Simple Baseline for Bayesian Uncertainty in Deep Learning' by Wesley Maddox, Timur Garipov, Pavel Izmailov, Dmitry Vetrov, and Andrew Gordon Wilson
...
def swag_parameters(module, params, no_cov_mat=True):
    for name in list(module._parameters.keys()):
        ...
        module.register_buffer("%s_mean" % name, data.new_zeros(data.size()))
        module.register_buffer("%s_sq_mean" % name, data.new_zeros(data.size()))

        if no_cov_mat is False:
            module.register_buffer(
                "%s_cov_mat_sqrt" % name, data.new_zeros((0, data.numel()))
            )
...
class SWAG(torch.nn.Module):
    def __init__(
        self, base, no_cov_mat=True, max_num_models=0, var_clamp=1e-30, device = torch.device("cuda"), *args, **kwargs
    ):
        ...
        self.base = base(*args, **kwargs, device = device)
        self.base.apply(
